...
mmod:boolean;
a:integer;
morskie:array[1..788] of integer;
...
for i:=1 to 788 do begin
  wspolx(i,x,y);
  if form1.image1.Canvas.Pixels[x,y]=clsilver then
  z:=inttostr(i);
  form1.numer2.Caption:=z;
end;
end;
...
if i=2 then begin
     try
      image1.canvas.Brush.color:=stringtocolor(label13.Caption);
      image1.Canvas.FloodFill(strtoint(label11.caption),strtoint(label12.caption),clblack,fsborder);
    except
    sleep(0);
  end;
  GetCursorPos(p);
  p.x:=p.x-panel1.Left-image1.left-form1.Left-8;
  p.y:=p.y-panel1.top-image1.top-form1.top-28;
  if strtoint(label14.Caption)=0 then begin
    c:=colortostring(image1.Canvas.Pixels[p.x,p.y]);
    if (c = 'clWhite') or (c='clRed') or (c='clYellow') then begin
      label13.Caption:=c;
      ...
      image1.Canvas.FloodFill(p.x,p.y,clblack,fsborder);
      if label5.caption='0' then begin
      memo1.Lines.Add('sasiedzi['+numer.caption+','+inttostr(a)+']:='+numer2.caption+';');
      inc(a);
      end else
      morskie[a]:=strtoint(numer2.Caption);
      mil:=mil+1;
      a:=a+1;
    end;
  end else
...
numer.caption:=inttostr(strtoint(numer.caption)+1);
wspolx(i,x,y);
image1.Canvas.Brush.Color:=clred;
image1.Canvas.FloodFill(x,y,clblack,fsborder);
i:=strtoint(numer.caption);
wspolx(i,x,y);
while form1.image1.Canvas.Pixels[x,y]=clred do begin
numer.caption:=inttostr(strtoint(numer.caption)+1);
...
begin
memo1.lines.savetofile('sasiedzi.txt');
end;
...
image1.Canvas.FloodFill(x,y,clblack,fsborder);
for z:=1 to mil do
  for s:=1 to mil do begin
    memo1.Lines.Add('sasiedzi['+inttostr(morskie[z])+','+inttostr(s)+']:='+inttostr(morskie[s])+';');
end;
 end;
procedure TForm1.Timer1Timer(Sender: TObject);
begin
label2.caption:=inttostr(a);
label5.caption:=booltostr(mmod);
end;
